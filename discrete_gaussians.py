...
def psi(beta, k, q, num_terms=100):
    """
    Computes the $\overline{\psi}$ value for a given integer k and parameter beta.

    Parameters:
    beta (float): A fixed parameter that controls the shape of the exponential function.
    k (int): The integer index for which to compute $\overline{\psi}$.
    ...
    Returns:
    float: The computed value of $\overline{\psi}$ for the given parameters.
    """
...
def create_empirical_distribution(beta, q, num_terms=100):
    ...
    Parameters:
    beta (float): A fixed parameter that controls the $\overline{\psi}$ function.
    ...
    """
    possible_values = np.arange(-q/2, q/2)  # Possible values: 0, 1, ..., q-1
    probabilities = np.array([psi(beta, k, q, num_terms) for k in possible_values])
